      ...
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
          {footerLinks.map((item) => (
            ...
            >
              <h4 className="text-[18px] font-medium text-white leading-[27px]">
                {item.title}
              </h4>
              <ul>
                {item.links.map((link, index) => (
                  <li
                    key={index}
                    className={`font-poppins font-normal text-base leading-[24px] text-dimWhite hover:text-secondary cursor-pointer duration-100 ${
